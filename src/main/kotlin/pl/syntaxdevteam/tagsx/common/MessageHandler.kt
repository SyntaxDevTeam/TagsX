...
class MessageHandler(private val plugin: TagsX) {
    ...
    */
    ...
    fun getListComponents(category: String, key: String): List<Component> {
        val list = messages.getStringList("$category.$key")
        return list.map { MiniMessage.miniMessage().deserialize(it, TagResolver.empty()) }
    }
